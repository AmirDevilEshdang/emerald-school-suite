  ...
  try {
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `شما یک دستیار هوشمند آموزشی هستید که به دانش‌آموزان مدرسه کمک می‌کنید. به فارسی پاسخ دهید. نام دانش‌آموز: ${studentName || "دانش‌آموز"}. پاسخ‌های کوتاه، دقیق و مفید بدهید. اگر سوال درسی باشد، توضیح واضح و ساده بدهید.`,
          },
          ...messages,
        ],
        stream: true,
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        return new Response(JSON.stringify({ error: "اعتبار سرویس هوش مصنوعی تمام شده است." }), {
